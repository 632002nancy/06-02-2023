...
const newBooking = async (select) => {
  ...
  const response = await fetch("http://localhost:1100/getdetails");
...
const allDetails = async (p) => {
  ...
  for (let i = 0; i < data.length; i++) {
    document.getElementById("allCustDetail").innerHTML += ` Customer Name:${data[i].customerName}   &nbsp;&nbsp; Phone Number: ${data[i]._id}&nbsp;&nbsp; Table Number: ${data[i].tableNumber} &nbsp;&nbsp; Time Slot: ${data[i].timeSlot}` + '<br>'
  }
}
